...
export default function UsersPage() {
  ...
  const fetchPage = useCallback((f: SearchFilters, page: number) => {
    setLoading(true);
    startTransition(async () => {
      ...
      if (res.ok) {
        ...
        setPagination({ page, pages: res.data.pages, total: res.data.total });
      }
      setLoading(false);
    });
  ...
  function handlePageChange(page: number) {
    if (filters) fetchPage(filters, page);
  }

  function refresh() {
    if (filters) fetchPage(filters, pagination.page);
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        {data.length === 0 && !loading ? (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-[var(--border)] py-20 text-center">
            ...
            <p className="text-sm text-[var(--foreground-muted)]">Search for citizens using the filters above</p>
          </div>
        ) : (
          <CitizenResultsTable
            data={data}
